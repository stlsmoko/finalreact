...
import { sessionKeepUris } from "@/lib/reaction-session";
...
export async function pruneReactionCache(keepUri?: string | null) {
  ...
  const keepList = [...sessionKeepUris(), keepUri]
    .filter((uri): uri is string => Boolean(uri))
    .map((uri) => uri.replace(/^file:\/\//, ""));
  const cutoff = Date.now() - 24 * 60 * 60 * 1000;

  function isKept(path: string) {
    return keepList.some((keep) => path === keep || keep.startsWith(path) || path.startsWith(keep));
  }
