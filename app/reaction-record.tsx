...
export default function ReactionRecordScreen() {
  ...
  const [reelVolume, setReelVolume] = useState(50); // 0 to 100
  const [micVolume, setMicVolume] = useState(150); // 0 to 350
  ...
  const bubbleCustomRadiusStyle =
    overlayStyle === "cutout"
      ? {
          borderTopLeftRadius: overlaySize / 2,
          borderTopRightRadius: overlaySize / 2,
          borderBottomLeftRadius: overlaySize * 0.4,
          borderBottomRightRadius: overlaySize * 0.4,
        }
      : {
  ...
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]} containerClassName="bg-black" safeAreaClassName="bg-black">
      <View
        ...
        style={styles.canvas}
      >
        {/* Minimal Top Bar matching website header */}
        <View style={styles.topBar}>
          ...
          <View style={styles.topActions}>
            <Pressable
              onPress={() => setIsAudioSheetOpen(true)}
              style={({ pressed }) => [styles.btnHeaderAction, pressed && styles.btnPressed]}
            >
            ...
            <Pressable
              onPress={handleChangeClip}
              disabled={isRecording}
              style={({ pressed }) => [styles.btnHeaderAction, (pressed || isRecording) && styles.btnPressed]}
            >
        ...
        <View style={styles.viewportWrapper}>
          <View style={styles.stageBox}>
            ...
            {/* Minimal Live Record Timer Pill */}
            {isRecording ? (
            ...
            {/* Camera Bubble Overlay over the Clip */}
            <View
              collapsable={false}
        ...
        {/* Bottom Floating Control Dock matching website */}
        <View style={styles.controlDock}>
          {/* Overlay Shape Floating Pill */}
          <View style={styles.shapePillBar}>
            <Pressable
              onPress={() => setOverlayStyle("circle")}
              style={[styles.shapeItem, overlayStyle === "circle" && styles.shapeItemActive]}
            >
              <Text style={[styles.shapeItemText, overlayStyle === "circle" && styles.shapeItemTextActive]}>
                Circle
              </Text>
            </Pressable>
            <Pressable
              onPress={() => setOverlayStyle("square")}
              style={[styles.shapeItem, overlayStyle === "square" && styles.shapeItemActive]}
            >
              <Text style={[styles.shapeItemText, overlayStyle === "square" && styles.shapeItemTextActive]}>
                Square
              </Text>
            </Pressable>
            <Pressable
              onPress={() => setOverlayStyle("cutout")}
              style={[styles.shapeItem, overlayStyle === "cutout" && styles.shapeItemActive]}
            >
              <Text style={[styles.shapeItemText, overlayStyle === "cutout" && styles.shapeItemTextActive]}>
                ✨ Cutout
              </Text>
            </Pressable>
          </View>
          ...
          <View style={styles.shutterBar}>
            {/* Left: Pause Background Reel Toggle */}
            <Pressable
              onPress={toggleSourcePause}
              disabled={!isRecording}
              style={({ pressed }) => [
                styles.dockSideBtn,
                isSourcePaused && styles.dockSideBtnPaused,
                !isRecording && styles.dockSideBtnDisabled,
                pressed && styles.btnPressed,
              ]}
            >
              <Text style={[styles.dockSideBtnIcon, isSourcePaused && styles.dockSideBtnIconPaused]}>
                {isSourcePaused ? "▶" : "⏸"}
              </Text>
            </Pressable>

            {/* Center: Iconic Apple Camera Shutter Button */}
            <Pressable
              onPress={toggleRecording}
              disabled={isCompositing}
              style={({ pressed }) => [styles.appleShutter, pressed && styles.btnPressed]}
            >
              <View style={[styles.shutterCore, isRecording && styles.shutterCoreRecording]} />
            </Pressable>

            {/* Right 1: Adjust Audio Modal Toggle */}
            <Pressable
              onPress={() => setIsAudioSheetOpen(true)}
              style={({ pressed }) => [styles.dockSideBtn, pressed && styles.btnPressed]}
            >
              <MaterialIcons name="tune" size={20} color="#FFFFFF" />
            </Pressable>

            {/* Right 2: Toggle/Flip Camera */}
            <Pressable
              onPress={handleFlipCamera}
              disabled={isRecording}
              style={({ pressed }) => [styles.dockSideBtn, isRecording && styles.dockSideBtnDisabled, pressed && styles.btnPressed]}
            >
        ...
        {/* Apple Glass Audio Control Bottom Sheet Modal */}
        <Modal
          visible={isAudioSheetOpen}
          transparent
          animationType="fade"
          onRequestClose={() => setIsAudioSheetOpen(false)}
        >
          <Pressable
            style={styles.audioSheetBackdrop}
            onPress={() => setIsAudioSheetOpen(false)}
          >
            <Pressable style={styles.audioSheet} onPress={(e) => e.stopPropagation()}>
              <View style={styles.audioSheetHeader}>
                ...
                <Pressable
                  onPress={() => setIsAudioSheetOpen(false)}
                  style={({ pressed }) => [styles.btnCloseSheet, pressed && styles.btnPressed]}
                >
              ...
              <View style={styles.audioChannelGroup}>
                ...
                <View style={styles.audioRow}>
                  ...
                  <View style={styles.audioSliderWrap}>
                    <Pressable
                      onPress={() => setIsReelMuted((m) => !m)}
                      style={[styles.audioIconBtn, isReelMuted && styles.audioIconBtnMuted]}
                    >
                      <Text style={styles.audioIconBtnText}>{isReelMuted ? "🔇" : "🔊"}</Text>
                    </Pressable>
                    <View style={styles.sliderContainer}>
                      <AppleAudioSlider
                        value={isReelMuted ? 0 : reelVolume}
                        max={100}
                        onChange={(val) => {
                          setReelVolume(val);
                          if (isReelMuted) setIsReelMuted(false);
                        }}
                      />
                    </View>
                  </View>
                ...
                <View style={styles.audioRow}>
                  ...
                  <View style={styles.audioSliderWrap}>
                    <Pressable
                      ...
                      style={[styles.audioIconBtn, isMicMuted && styles.audioIconBtnMuted]}
                    >
                ...
                {/* Live Vocal Level Visualizer Meter */}
                <View style={styles.micMeterContainer}>
                  ...
                  <View style={styles.micMeterTrack}>
                    <View
                      style={[
                        styles.micMeterFill,
                        { width: isRecording && !isMicMuted ? "65%" : "8%" },
                      ]}
                    />
                  </View>
                </View>
                ...
                <View style={styles.audioToggleRow}>
                  ...
                  <Switch
                    value={isAutoDucking}
                    onValueChange={setIsAutoDucking}
